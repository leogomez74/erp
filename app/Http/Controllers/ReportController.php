    ...
    public function incomeSummary(Request $request)
    {
        if(\Auth::user()->can('income report'))
        {
            ...
            foreach($types as $type){
                ...
                if($type->name=='Income'){
                    $category = ChartOfAccount::where('type', $type->id)->where('created_by', '=', \Auth::user()->creatorId())->get();
                    $category->prepend('Select Accounts', '');
            ...
            {
                $tmp             = [];
                $tmp['category'] = !empty(ChartOfAccount::where('id', '=', $cat_id)->first()) ? ChartOfAccount::where('id', '=', $cat_id)->first()->name : '';
                $tmp['data']     = [];
                for($i = 1; $i <= 12; $i++)
                {
            ...
            foreach($invoiceTmpArray as $cat_id => $record)
            {
                ...
                $invoice             = [];
                $invoice['category'] = !empty(ChartOfAccount::where('id', '=', $cat_id)->first()) ? ChartOfAccount::where('id', '=', $cat_id)->first()->name : '';
                $invoice['data']     = [];
                for($i = 1; $i <= 12; $i++)
                {
...
//dd($incomeTotal);
        ...
        if(\Auth::user()->can('expense report'))
        {
            ...
            foreach($types as $type){
                ...
                if($type->name=='Expenses'){
                    $category = ChartOfAccount::where('type', $type->id)->where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id');
                    $category->prepend('Select Accounts', '');
            ...
            if(!empty($request->category))
            {
                ...
                $cat                = ChartOfAccount::find($request->category);
                $filter['category'] = !empty($cat) ? $cat->name : '';
            }
            ...
            foreach($billTmpArray as $cat_id => $record)
            {
                ...
                $bill             = [];
                $bill['category'] = !empty(ChartOfAccount::where('id', '=', $cat_id)->first()) ? ChartOfAccount::where('id', '=', $cat_id)->first()->name : '';
                $bill['data']     = [];
                for($i = 1; $i <= 12; $i++)
                {
    ...
    public function incomeVsExpenseSummary(Request $request)
    {
        if(\Auth::user()->can('income vs expense report'))
        {
            ...
            foreach($types as $type){
                ...
                if($type->name=='Expenses'){
                    $expense = ChartOfAccount::where('type', $type->id)->where('created_by', '=', \Auth::user()->creatorId())->get();
            ...
            $categories = $expense->merge($income);
            $category =  $categories->merge($liability)->pluck("name","id");
            $category->prepend('Select Account', '');
    ...
    public function accountStatement(Request $request)
    {
        if(\Auth::user()->can('statement report'))
        {
            ...
            if($request->type == 'revenue' || !isset($request->type))
            {
                ...
                $revenueAccounts2 = InvoicePayment::select("invoice_payments.account_id", "ca.name")->leftJoin('chart_of_accounts as ca', 'invoice_payments.account_id', '=', 'ca.id')->groupBy('invoice_payments.account_id')->selectRaw('sum(amount) as total')->leftJoin("invoices as b","b.id","=","invoice_payments.invoice_id")->where('b.created_by', '=', \Auth::user()->creatorId());

                $revenueAccounts = Revenue::select('bank_accounts.id', 'bank_accounts.name')->leftjoin('chart_of_accounts as bank_accounts', 'revenues.account_id', '=', 'bank_accounts.id')->groupBy('revenues.account_id')->selectRaw('sum(amount) as total')->where('revenues.created_by', '=', \Auth::user()->creatorId());

                $revenues2 = InvoicePayment::select("invoice_payments.id","date","amount","description")->leftJoin("invoices","invoices.id","=","invoice_payments.invoice_id")->where('invoices.created_by', '=', \Auth::user()->creatorId());

                $revenues = Revenue::select("revenues.id","date","amount","description")->where('revenues.created_by', '=', \Auth::user()->creatorId())->orderBy('id', 'desc');
            }

            if($request->type == 'payment')
            {
                $paymentAccounts2 = BillPayment::select("bill_payments.account_id", "ca.name","b.created_by")->leftJoin('chart_of_accounts as ca', 'bill_payments.account_id', '=', 'ca.id')->groupBy('bill_payments.account_id')->selectRaw('sum(amount) as total')->leftJoin("bills as b","b.id","=","bill_payments.bill_id")->where('b.created_by', '=', \Auth::user()->creatorId());

                $paymentAccounts = Payment::select('bank_accounts.id', 'bank_accounts.name','payments.created_by')->leftjoin('chart_of_accounts as bank_accounts', 'payments.account_id', '=', 'bank_accounts.id')->groupBy('payments.account_id')->selectRaw('sum(amount) as total')->where('payments.created_by', '=', \Auth::user()->creatorId());                

                $payments2 = BillPayment::select("bill_payments.id","date","amount","description","bill_payments.account_id","bills.created_by")->leftJoin("bills","bills.id","=","bill_payments.bill_id")->where('bills.created_by', '=', \Auth::user()->creatorId());
                
                $payments = Payment::select("payments.id","date","amount","description","payments.account_id","payments.created_by")->where('payments.created_by', '=', \Auth::user()->creatorId())->orderBy('id', 'desc');
            ...
            if(!empty($request->account))
            {
                if($request->type == 'revenue' || !isset($request->type))
                {
                    ...
                    $revenues2->where('account_id', $request->account);

                
                    $revenues2->where('invoices.created_by', '=', \Auth::user()->creatorId());

                
      
                    $revenueAccounts2->where('account_id', $request->account);

                    $revenueAccounts2->where('b.created_by', '=', \Auth::user()->creatorId());
                ...
                if($request->type == 'payment')
                {
                    ...
                    $payments2->where('account_id', $request->account);
                    $payments2->where('created_by', '=', \Auth::user()->creatorId());
                    ...
                    $paymentAccounts2->where('account_id', $request->account);
           
                    $paymentAccounts2->where('b.created_by', '=', \Auth::user()->creatorId());
      
                }


                $bankAccount       = ChartOfAccount::find($request->account);
                $filter['account'] = !empty($bankAccount) ? $bankAccount->name : '';
            ...
            if($request->type == 'revenue' || !isset($request->type))
            {
                ...
                $revenueAccounts->where('revenues.created_by', '=', \Auth::user()->creatorId());
                $reportData['revenueAccounts'] = $revenueAccounts->get();//add union here

            }

            if($request->type == 'payment')
            {
                $reportData['payments'] = $payments->union($payments2)->get();

                $paymentAccounts->where('payments.created_by', '=', \Auth::user()->creatorId());
                $paymentAccounts2->where('b.created_by', '=', \Auth::user()->creatorId());
           
                $reportData['paymentAccounts'] = $paymentAccounts->union($paymentAccounts2)->get(); //add union here
    ...
    public function account(Request $request){
        ...
        $incomes = Revenue::selectRaw('sum(amount) as amount,account_id,chart_of_accounts.name as name_account,chart_of_account_types.name as name_type,chart_of_account_sub_types.name as name_sub_type');
        $incomes->join('chart_of_accounts','chart_of_accounts.id','=','revenues.account_id');
        $incomes->join('chart_of_account_types','chart_of_account_types.id','=','chart_of_accounts.type');
        $incomes->join('chart_of_account_sub_types','chart_of_account_sub_types.id','=','chart_of_accounts.sub_type');

        //$incomes->where('created_by', '=', \Auth::user()->creatorId());
        $incomes->whereRAW('YEAR(date) =?', [$year]);
        $incomes->groupBy('account_id');
        $incomes = $incomes->get();
        $data["totalIncome"]=0;
        foreach($incomes as $amount){
            //dd($amount['amount']);
            $data["totalIncome"]=$data["totalIncome"]+$amount['amount'];
        }

        //---------------------------------PAYMENT EXPENSE-----------------------------------

        $expenses = Payment::selectRaw('sum(amount) as amount,account_id,chart_of_accounts.name as name_account,chart_of_account_types.name as name_type,chart_of_account_sub_types.name as name_sub_type');
        $expenses->join('chart_of_accounts','chart_of_accounts.id','=','payments.account_id');
        $expenses->join('chart_of_account_types','chart_of_account_types.id','=','chart_of_accounts.type');
        $expenses->join('chart_of_account_sub_types','chart_of_account_sub_types.id','=','chart_of_accounts.sub_type');
        //$expenses->where('created_by', '=', \Auth::user()->creatorId());
        $expenses->whereRAW('YEAR(date) =?', [$year]);
        $expenses->groupBy('account_id');
        $expenses = $expenses->get();
        $data["totalExpense"]=0;
        foreach($expenses as $amount){
            //dd($amount['amount']);
            $data["totalExpense"]=$data["totalExpense"]+$amount['amount'];
        }
        //dd($data);
        ...
        $data['incomes']=$incomes;
        $data['expenses']=$expenses;
        $data['TotalNetProfit']=$data["totalIncome"]-$data["totalExpense"];
   ...
   public function profitLossTotal(Request $request){
        ...
        if(isset($request->start_date) && isset($request->end_date) && isset($request->start_date1) && isset($request->end_date1))
        {
            ...
            $data['start_date']=Carbon::parse($request->start_date)->format('M d, Y');
            $data['end_date']=Carbon::parse($request->end_date)->format('M d, Y');
            $data['start_date1']=Carbon::parse($request->start_date1)->format('M d, Y');
            $data['end_date1']=Carbon::parse($request->end_date1)->format('M d, Y');
        }
        else
        {
            ...
            $data['start_date1']=date('M d, Y');
            $data['end_date1']=date('M d, Y');  
            
        }
        $data['yearList']  = $this->yearList();

        // -------------------------------REVENUE INCOME RANGE DATE-------------------------------------------------

        $incomes = Revenue::select('account_id','account_id as account_id_new','chart_of_accounts.name as name_account','chart_of_account_types.name as name_type','chart_of_account_sub_types.name as name_sub_type',DB::raw("ifnull((Select SUM(amount) as amount from revenues where account_id=account_id_new and date between '$start_date' and '$end_date' ),0) as amount_date1"),DB::raw("ifnull((Select SUM(amount) as amount from revenues where account_id=account_id_new and date between '$start_date1' and '$end_date1'),0) as amount_date2"),DB::raw("ifnull((Select SUM(amount) as amount from revenues where account_id=account_id_new and date between '$start_date1' and '$end_date1' ),0) - ifnull((Select SUM(amount) as amount from revenues where account_id=account_id_new and date between '$start_date' and '$end_date'),0) as total_change"),DB::raw("ifnull(CAST((ifnull((Select SUM(amount) as amount from revenues where account_id=account_id_new and date between '$start_date1' and '$end_date1' ),0) - ifnull((Select SUM(amount) as amount from revenues where account_id=account_id_new and date between '$start_date' and '$end_date'),0)/ifnull((Select SUM(amount) as amount from revenues where account_id=account_id_new and date between '$start_date' and '$end_date'),0))*100 as Decimal(6,2)),100) as porcentaje_change"));
        $incomes->join('chart_of_accounts','chart_of_accounts.id','=','revenues.account_id');
        $incomes->join('chart_of_account_types','chart_of_account_types.id','=','chart_of_accounts.type');
        $incomes->join('chart_of_account_sub_types','chart_of_account_sub_types.id','=','chart_of_accounts.sub_type');
        //$incomes->where('revenues.created_by', '=', \Auth::user()->creatorId());
        $incomes->whereBetween('date',[$start_date,$end_date]);
        $incomes->orWhereBetween('date',[$start_date1,$end_date1]);
        $incomes->groupBy('account_id');
        $incomes = $incomes->get();
        //dd($incomes);
        $data["totalIncomeRangeDate1"]=0;
        $data["totalIncomeRangeDate2"]=0;
        $data["totalChange"]=0;

        foreach($incomes as $amount){
            if(empty($amount['amount_date1'])){$amount['amount_date1']=0;}else{$amount['amount_date1'];}
            if(empty($amount['amount_date2'])){$amount['amount_date2']=0;}else{$amount['amount_date2'];}
            $data["totalIncomeRangeDate1"]=$data["totalIncomeRangeDate1"]+$amount['amount_date1'];
            $data["totalIncomeRangeDate2"]=$data["totalIncomeRangeDate2"]+$amount['amount_date2'];
            $data["totalChange"]=$data["totalChange"]+$amount['total_change'];

        }
        // -------------------------------PAYMENT INCOME RANGE DATE-------------------------------------------------

        $expenses = Payment::select('account_id','account_id as account_id_new','chart_of_accounts.name as name_account','chart_of_account_types.name as name_type','chart_of_account_sub_types.name as name_sub_type',DB::raw("ifnull((Select SUM(amount) as amount from payments where account_id=account_id_new and date between '$start_date' and '$end_date' ),0) as amount_date1"),DB::raw("ifnull((Select SUM(amount) as amount from payments where account_id=account_id_new and date between '$start_date1' and '$end_date1'),0) as amount_date2"),DB::raw("ifnull((Select SUM(amount) as amount from payments where account_id=account_id_new and date between '$start_date1' and '$end_date1' ),0) - ifnull((Select SUM(amount) as amount from payments where account_id=account_id_new and date between '$start_date' and '$end_date'),0) as total_change"),DB::raw("CAST((ifnull((Select SUM(amount) as amount from payments where account_id=account_id_new and date between '$start_date1' and '$end_date1' ),0) - ifnull((Select SUM(amount) as amount from payments where account_id=account_id_new and date between '$start_date' and '$end_date'),0)/ifnull((Select SUM(amount) as amount from payments where account_id=account_id_new and date between '$start_date' and '$end_date'),0))*100 as Decimal(6,2)) as porcentaje_change"));
        $expenses->join('chart_of_accounts','chart_of_accounts.id','=','payments.account_id');
        $expenses->join('chart_of_account_types','chart_of_account_types.id','=','chart_of_accounts.type');
        $expenses->join('chart_of_account_sub_types','chart_of_account_sub_types.id','=','chart_of_accounts.sub_type');
        //$incomes->where('revenues.created_by', '=', \Auth::user()->creatorId());
        $expenses->whereBetween('date',[$start_date,$end_date]);
        $expenses->orWhereBetween('date',[$start_date1,$end_date1]);
        $expenses->groupBy('account_id');
        $expenses = $expenses->get();
        $data["totalExpensesRangeDate1"]=0;
        $data["totalExpensesRangeDate2"]=0;
        $data["totalExpensesChange"]=0;

        foreach($expenses as $amount){
            if(empty($amount['amount_date1'])){$amount['amount_date1']=0;}else{$amount['amount_date1'];}
            if(empty($amount['amount_date2'])){$amount['amount_date2']=0;}else{$amount['amount_date2'];}
            $data["totalExpensesRangeDate1"]=$data["totalExpensesRangeDate1"]+$amount['amount_date1'];
            $data["totalExpensesRangeDate2"]=$data["totalExpensesRangeDate2"]+$amount['amount_date2'];
            $data["totalExpensesChange"]=$data["totalExpensesChange"]+$amount['total_change'];

        }

        $data['incomes']=$incomes;
        $data['expenses']=$expenses;
        return view('report.profit_loss',$data);
