...
class InvoiceController extends Controller
{
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            foreach ($types as $type) {
                if ($type->name == 'Income') {
                    ...
                    $category->prepend('Select Chart of accounts', '');
                }
            }
    ...
    public function store(Request $request): RedirectResponse
    {
        if (\Auth::user()->can('create invoice')) {
            ...
            return redirect()->route('invoice.show', \Crypt::encrypt($invoice->id))->with('success', __('Invoice successfully created.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function sent($id): RedirectResponse
    {
        if (\Auth::user()->can('send invoice')) {
            ...
            if ($setings['customer_invoice_send'] == 1) {
                $invoice = Invoice::where('id', $id)->first();
...
//        dd($invoice);
        ...
        if ($invoice) {
            $invoiceProduct = InvoiceProduct::where('invoice_id', $invoice_id)->get();
            foreach ($invoiceProduct as $product) {
                $total_amount += $total_amount + ($product->price * $product->quantity) - $product->discount;
            }
        }
        $data = [
            ...
            'category' => 'Otro',
            'amount' => $total_amount,
            'observation' => 'cobro creado de factura en https://erp.ssc.cr/invoice/'.Crypt::encrypt($invoice_id),
        ...
        return redirect()->back()->with('success', __('Invoice to charges successfully.'));
    }
}
