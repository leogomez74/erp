...
class VenderController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create vender')) {
            ...
            //pay type array added  by ahixel rojas at 22/09/2022 19:00 to select input
            $pay_type = ['0' => 'Cash', '1' => 'On Credit'];
            // Create by Ahixel rojas at 28/09/2022 15:00 to set country field as costa rica default
            $country = 'Costa Rica';
            ///////////////////////////////////////////////////////////////////////////
            return view('vender.create', compact('customFields', 'pay_type', 'country'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create vender')) {
            ...
            if ($total_vendor < $plan->max_venders || $plan->max_venders == -1) {
                ...
                //shipping account number and pay type added  by ahixel rojas at 22/09/2022 19:00 to Create vendor form
                $vender->account_number = $request->account_number;
                $vender->pay_type = $request->pay_type;
            ...
            //Send mail commented by ahixel rojas at 22/09/2022 19:00 to prevent to send mail messages
//                        try {
//
...
//                        }
    ...
    public function edit($id)
    {
        if (\Auth::user()->can('edit vender')) {
            ...
            return view('vender.edit', compact('vender', 'customFields', 'pay_type'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function update(Request $request, Vender $vender)
    {
        if (\Auth::user()->can('edit vender')) {
            ...
            $vender->shipping_zip = $request->shipping_zip;
            $vender->shipping_address = $request->shipping_address;
            //shipping account number and pay type added  by ahixel rojas at 22/09/2022 19:00 to update vendor form
            $vender->account_number = $request->account_number;
            $vender->pay_type = $request->pay_type;
...
//            $company_location  = array_search($vendor[26], $countryStausArray);
            ...
            $vendorData->password = Hash::make($vendor[3]);
