...
//                $user->userDefaultData();
                ...
                if ($request->gender == 'yes') {
                    foreach ($request->companies as $company) {
                        $company_id_name = explode('-', $company);
                        $company1['email'] = $request->name.'@'.$company_id_name[0].'.cr';
                        $company1['password'] = Hash::make($request->password);
                        $company1['type'] = 'EMPLOYEE';
                        $company1['default_pipeline'] = 1;
                        $company1['plan'] = 1;
                        $company1['lang'] = ! empty($default_language) ? $default_language->value : '';
                        $company1['created_by'] = $company_id_name[1];
                        $company1['created_id'] = $user->id;
                        $company1['plan'] = Plan::first()->id;
                        $company1['name'] = $request->name.'-'.$company_id_name[0];
                        $user_r = User::create($company1);
                        $role_r = Role::findByName('company');
                        $user_r->assignRole($role_r);
                    }
                }
            //dd($companies);
            } else {
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit user')) {
            ...
            $customFields = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'user')->get();
            $companies = User::where('type', 'company')->get();
            $companies_users = User::where('created_id', $id)->get()->toArray();

            // echo "<pre>";print_r($companies_users);echo "</pre>";exit;
            return view('user.edit', compact('user', 'roles', 'customFields', 'companies', 'companies_users'));
        } else {
            return redirect()->back();
        }
    }
    ...
    public function update(Request $request, $id): RedirectResponse
    {
        if (\Auth::user()->can('edit user')) {
            if (\Auth::user()->type == 'super admin') {
                ...
                if ($request->gender == 'yes') {
                    User::where(['created_id' => $id])->delete();
                    foreach ($request->companies as $company) {
                        $company_id_name = explode('-', $company);
                        $company1['email'] = $request->name.'@'.$company_id_name[0].'.cr';
                        $company1['password'] = Hash::make($request->password);
                        $company1['type'] = 'EMPLOYEE';
                        $compan1y['default_pipeline'] = 1;
                        $company1['plan'] = 1;
                        $company1['lang'] = ! empty($default_language) ? $default_language->value : '';
                        $company1['created_by'] = $company_id_name[1];
                        $company1['created_id'] = $id;
                        $company1['plan'] = Plan::first()->id;
                        $company1['name'] = $request->name.'-'.$company_id_name[0];
                        $user_r = User::create($company1);
                        $role_r = Role::findByName('company');
