...
class ProductServiceController extends Controller
{
    public function index(Request $request)
    {
        if (\Auth::user()->can('manage product & service')) {
            ...
            if (! empty($request->category)) {
                $productServices = ProductService::where('created_by', '=', \Auth::user()->creatorId())->where('category_id', $request->category)->orderByDesc('id')->get();
            } else {
                $productServices = ProductService::where('created_by', '=', \Auth::user()->creatorId())->orderByDesc('id')->get();
                //dd($productServices);
            }
    ...
    public function create()
    {
        if (\Auth::user()->can('create product & service')) {
            ...
            return view('productservice.create', compact('category', 'unit', 'tax', 'customFields', 'currencies'));
        } else {
    ...
    public function store(Request $request): RedirectResponse
    {
        if (\Auth::user()->can('create product & service')) {
            ...
            $productService->sku = $request->sku.'-'.date('Y-m-d', strtotime($request->issue_date));
            ...
            if ($productService->type == 'service') {
            ...
            $productPurchase->issue_date = $request->issue_date;
            $productPurchase->currency = $request->currency;
    ...
    public function update(Request $request, $id): RedirectResponse
    {
        if (\Auth::user()->can('edit product & service')) {
            ...
            if ($productService->created_by == \Auth::user()->creatorId()) {
                ...
                ProductPurchase::where('product_id', $productService->id)->update(['sale_price' => $request->sale_price, 'purchase_price' => $request->purchase_price, 'quantity' => $request->quantity]);
