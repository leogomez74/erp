    ...
    public function account_dashboard_index(): Renderable
    {
        if (Auth::check()) {
            if (Auth::user()->type == 'super admin') {
                ...
                $incomeCategory = $income->merge($liability);
                ...
                foreach ($types as $type) {
                    if ($type->name == 'Expenses') {
                        $expenses = ChartOfAccount::where('type', $type->id)->where('created_by', '=', \Auth::user()->creatorId())->get();
                    }
                    if ($type->name == 'Liabilities') {
                        $liability = ChartOfAccount::where('type', $type->id)->where('created_by', '=', \Auth::user()->creatorId())->get();
                    }
                }
                $expenseCategory = $expenses->merge($liability);
        ...
        if (Auth::check()) {
            if (\Auth::user()->can('show hrm dashboard')) {
                ...
                if ($user->type != 'client' && $user->type != 'company') {
                    ...
                    if ($user->email == 'leonardo@gomez.cr') {
                        $events = Event::get();
                        $arrEvents = [];
                        ...
                        $emp = User::where('type', '!=', 'client')->where('type', '!=', 'company')->get();
                        $countEmployee = count($emp);

                        $user = User::where('type', '!=', 'client')->where('type', '!=', 'company')->get();
