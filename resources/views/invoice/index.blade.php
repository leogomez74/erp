                                    ...
                                    @if (Gate::check('edit invoice') || Gate::check('delete invoice') || Gate::check('show invoice'))
                                        <td class="Action">
                                                ...
                                                @php $invoiceID= Crypt::encrypt($invoice->id); @endphp
                                                    ...
                                                    @if($invoice->status==0)
                                                    <div class="action-btn bg-warning ms-2">
                                                        <a href="{{ route('invoice.mark-sent',$invoice->id) }}" class="mx-3 btn btn-sm align-items-center" data-original-title="{{ __('Mark as sent') }}" data-bs-toggle="tooltip" title="Mark as sent"
                                                           data-original-title="{{ __('Mark as sent') }}"><i class="ti ti-check text-white"></i></a>
                                                    </div>
                                                    @endif
                                                    @if($invoice->status!=0&&$invoice->status!=4)
                                                        <div class="action-btn bg-info ms-2">
                                                        @can('create payment invoice')
                                                            <a href="#" data-url="{{ route('invoice.payment',$invoice->id) }}" data-ajax-popup="true"  class="mx-3 btn btn-sm align-items-center " data-original-title="{{ __('Add Payment') }}" data-bs-toggle="tooltip" title="Add Payment"
                                                               data-original-title="{{ __('Add payment') }}"><i class="ti ti-report-money text-white"></i></a> <br>
                                                        @endcan
                                                        </div>
                                                    @endif
                                                    @can('manage charges')
                                                        @if($invoice->status !=5&&$invoice->status !=4&&$invoice->status !=3)
                                                        <div class="action-btn bg-info ms-2">
                                                           {!! Form::open(['method' => 'post', 'route' => ['invoice.invoicetocharge', $invoice->id], 'id' => 'invoicetocharge-form-' . $invoice->id]) !!}

                                                            <a href="#" class="mx-3 btn btn-sm align-items-center bs-pass-para" data-toggle="tooltip"
                                                               data-original-title="{{ __('Invoice To Charge') }}" data-bs-toggle="tooltip" title="Invoice to Charge"
                                                               data-original-title="{{ __('Delete') }}"
                                                               data-confirm="You want to confirm this action. Press Yes to continue or Cancel to go back"
                                                               data-confirm-yes="document.getElementById('invoicetocharge-form-{{ $invoice->id }}').submit();">
