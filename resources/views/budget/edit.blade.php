...
@push('script-page')
    ...
    <script>
        ...
        //Assets Total
        $(document).on('keyup', '.assets_data', function () {
            //category wise total
            var el = $(this).parent().parent();
            var inputs = $(el.find('.assets_data'));

            var totalassets = 0;
            for (var i = 0; i < inputs.length; i++) {
                var price = $(inputs[i]).val();
                totalassets = parseFloat(totalassets) + parseFloat(price);
            }
            el.find('.totalAssets').html(totalassets);

            // month wise total //
            var month_assets = $(this).data('month');
            var month_inputs = $(el.parent().find('.' + month_assets+'_assets'));
            var month_totalassets = 0;
            for (var i = 0; i < month_inputs.length; i++) {
                var month_price = $(month_inputs[i]).val();
                month_totalassets = parseFloat(month_totalassets) + parseFloat(month_price);
            }
            var month_total_assets = month_assets + '_total_assets';
            el.parent().find('.' + month_total_assets).html(month_totalassets);

            //all total //
            var total_inputs = $(el.parent().find('.totalAssets'));
            console.log(total_inputs)
            var assets = 0;
            for (var i = 0; i < total_inputs.length; i++) {
                var price = $(total_inputs[i]).html();
                assets = parseFloat(assets) + parseFloat(price);
            }
            el.parent().find('.assets').html(assets);

        })
        //Liabilitie Total
        $(document).on('keyup', '.liabilitie_data', function () {
            //category wise total
            var el = $(this).parent().parent();
            var inputs = $(el.find('.liabilitie_data'));

            var totalliabilitie = 0;
            for (var i = 0; i < inputs.length; i++) {
                var price = $(inputs[i]).val();
                totalliabilitie = parseFloat(totalliabilitie) + parseFloat(price);
            }
            el.find('.totalLiabilitie').html(totalliabilitie);

            // month wise total //
            var month_liabilitie = $(this).data('month');
            var month_inputs = $(el.parent().find('.' + month_liabilitie+'_liabilitie'));
            var month_totalliabilitie = 0;
            for (var i = 0; i < month_inputs.length; i++) {
                var month_price = $(month_inputs[i]).val();
                month_totalliabilitie = parseFloat(month_totalliabilitie) + parseFloat(month_price);
            }
            var month_total_liabilitie = month_liabilitie + '_total_liabilitie';
            el.parent().find('.' + month_total_liabilitie).html(month_totalliabilitie);

            //all total //
            var total_inputs = $(el.parent().find('.totalLiabilitie'));
            console.log(total_inputs)
            var liabilitie = 0;
            for (var i = 0; i < total_inputs.length; i++) {
                var price = $(total_inputs[i]).html();
                liabilitie = parseFloat(liabilitie) + parseFloat(price);
            }
            el.parent().find('.liabilitie').html(liabilitie);

        })
        //Equity Total
        $(document).on('keyup', '.equity_data', function () {
            //category wise total
            var el = $(this).parent().parent();
            var inputs = $(el.find('.equity_data'));

            var totalequity = 0;
            for (var i = 0; i < inputs.length; i++) {
                var price = $(inputs[i]).val();
                totalequity = parseFloat(totalequity) + parseFloat(price);
            }
            el.find('.totalEquity').html(totalequity);

            // month wise total //
            var month_equity = $(this).data('month');
            var month_inputs = $(el.parent().find('.' + month_equity+'_equity'));
            var month_totalequity = 0;
            for (var i = 0; i < month_inputs.length; i++) {
                var month_price = $(month_inputs[i]).val();
                month_totalequity = parseFloat(month_totalequity) + parseFloat(month_price);
            }
            var month_total_equity = month_equity + '_total_equity';
            el.parent().find('.' + month_total_equity).html(month_totalequity);

            //all total //
            var total_inputs = $(el.parent().find('.totalEquity'));
            console.log(total_inputs)
            var equity = 0;
            for (var i = 0; i < total_inputs.length; i++) {
                var price = $(total_inputs[i]).html();
                equity = parseFloat(equity) + parseFloat(price);
            }
            el.parent().find('.equity').html(equity);
            ...
            <div class="card-body table-border-style">
                ...
                <div class="table-responsive budget_plan d-block"  id="monthly">
                    <table class="table mb-0" id="dataTable-manual">
                        ...
                        <!-------------------------------Assets--------------------------------------->
                        <tr>
                            <th colspan="14" class="text-dark light_blue"><span>{{__('Assets :')}}</span></th>
                        </tr>

                        @foreach ($assetsproduct as $productService)
                            <tr>
                                <td>{{__($productService->name)}}</td>
                                @foreach($monthList as $month)
                                    <td>
                                        <input type="number" class="form-control pl-1 pr-1 assets_data {{$month}}_assets" data-month="{{$month}}" name="assets[{{$productService->id}}][{{$month}}]" value="{{!empty($budget['assets_data'][$productService->id][$month])?$budget['assets_data'][$productService->id][$month]:0}}" id="assets_data_{{$month}}">
                                    </td>
                                @endforeach
                                <td class="totalAssets text-dark">
                                    0.00
                                </td>
                            </tr>
                        @endforeach
                        ...
                        <tr>
                            ...
                            @foreach($monthList as $month)
                                <td>
                                    <span class="{{$month}}_total_assets text-dark">0.00</span>
                                </td>
                            @endforeach
                            <td>
                                <span class="assets text-dark">0.00</span>
                            </td>
                        </tr>

                        <!-------------------------------Liabilitie--------------------------------------->
                    <tr>
                            <th colspan="14" class="text-dark light_blue"><span>{{__('Liabilitie :')}}</span></th>
                        </tr>

                        @foreach ($liabilitieproduct as $productService)
                            <tr>
                                <td>{{__($productService->name)}}</td>
                                @foreach($monthList as $month)
                                    <td>
                                        <input type="number" class="form-control pl-1 pr-1 liabilitie_data {{$month}}_liabilitie" data-month="{{$month}}" name="liabilitie[{{$productService->id}}][{{$month}}]" value="{{!empty($budget['liabilitie_data'][$productService->id][$month])?$budget['liabilitie_data'][$productService->id][$month]:0}}" id="liabilitie_data_{{$month}}">
                                    </td>
                                @endforeach
                                <td class="totalLiabilitie text-dark">
                                    0.00
                                </td>
                            </tr>
                        @endforeach
                        ...
                        <tr>
                            ...
                            @foreach($monthList as $month)
                                <td>
                                    <span class="{{$month}}_total_liabilitie text-dark">0.00</span>
                                </td>
                            @endforeach
                            <td>
                                <span class="liabilitie text-dark">0.00</span>
                            </td>
                        </tr>
                         <!-------------------------------Equity--------------------------------------->
                    <tr>
                            <th colspan="14" class="text-dark light_blue"><span>{{__('Equity :')}}</span></th>
                        </tr>

                        @foreach ($equityproduct as $productService)
                            <tr>
                                <td>{{__($productService->name)}}</td>
                                @foreach($monthList as $month)
                                    <td>
                                        <input type="number" class="form-control pl-1 pr-1 equity_data {{$month}}_equity" data-month="{{$month}}" name="equity[{{$productService->id}}][{{$month}}]" value="{{!empty($budget['equity_data'][$productService->id][$month])?$budget['equity_data'][$productService->id][$month]:0}}"id="equity_data_{{$month}}">
                                    </td>
                                @endforeach
                                <td class="totalEquity text-dark">
                                    0.00
                                </td>
                            </tr>
                        @endforeach
                        ...
                        <tr>
                            ...
                            @foreach($monthList as $month)
                                <td>
                                    <span class="{{$month}}_total_equity text-dark">0.00</span>
                                </td>
                            @endforeach
                            <td>
                                <span class="equity text-dark">0.00</span>
                            </td>
                        </tr>
                        </tbody>
