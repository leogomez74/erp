...
@section('content')
    ...
    <div class="row">
    <ul class="nav nav-pills mb-3 text-center" id="pills-tab" role="tablist" style="display:flex;justify-content:center">
@foreach($chartAccounts as $type=>$accounts)

  <li class="nav-item " role="presentation">
    <button class="nav-link {{$type==array_key_first($chartAccounts) ? 'active':''}}" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-{{$type}}" type="button" role="tab" aria-controls="pills-home" aria-selected="true">{{__($type)}} <span class="badge bg-info rounded-pill">{{count($accounts)}}</span></button>
  </li>
@endforeach
...
@foreach($chartAccounts as $type=>$accounts)
  <div class="tab-pane fade show {{$type==array_key_first($chartAccounts) ? 'active':''}}" id="pills-{{$type}}" role="tabpanel" aria-labelledby="pills-home-tab">
                            ...
                            <table class="table">
                                ...
                                @foreach ($accounts as $account)
                                    <tr>
                                        ...
                                        <td>
                                            @if(!empty($account->balance) && $account->balance<0)
                                                {{'-'.\Auth::user()->priceFormat(abs($account->balance))}}
                                            @elseif(!empty($account->balance()) && $account->balance>0)
                                        ...
                                        <td>
                                        {{\Auth::user()->priceFormat(abs($account->opening_balance))}}
                                        </td>
                                        <td>
